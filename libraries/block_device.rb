module BlockDevice
  ...
  def assemble_raid(raid_device, options)
  ...
  def create_raid(raid_device, options)
  ...
  def set_read_ahead(raid_device, ahead_option)
  ...
  def lvm_device(raid_device)
    "/dev/#{lvm_volume_group(raid_device)}/lvm#{raid_device.match(/\d+/)[0]}"
  end

  def lvm_volume_group(raid_device)
    "lvm-raid-#{raid_device.match(/\d+/)[0]}"
  end

  def existing_lvm_at?(lvm_device)
    lvms = `lvscan`
  ...
  def create_lvm(raid_device, options)
  ...
  def lvm_physical_group_exists?(raid_device)
    pvscan = `pvscan`
  ...
  def lvm_volume_group_exists?(raid_device)
    vgscan = `vgscan`
  ...
  def lvm_volume_exits?(raid_device)
  ...
  def translate_device_names(devices, skip = 0)
